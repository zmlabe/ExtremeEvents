"""
Try sample code to classify the data into 2 century-scale periods by giving the
...
for sis,singlesimulation in enumerate(datasetsingle):
    ...
    for seas in range(len(seasons)):
            ...
            for loop in ([0]): # (0,1,2,3,4,5):
                ...
                #     ### Loop over folds
                    for loop in np.arange(0,foldsN): 
                        ...
                        if(rm_standard_dev==True):
                            savename = savename + '_EnsembleSTD' 
                            savenameModelTestTrain = savenameModelTestTrain + '_EnsembleSTD'
        ...
        # np.savetxt(directoryoutput + 'training_Century_%s_%s_%s_%s_iterations%s_v2.txt' % (variq,monthlychoice,reg_name,dataset,iterations[0]),YpredTrain)
        # np.savetxt(directoryoutput + 'testing_Century_%s_%s_%s_%s_iterations%s_v2.txt' % (variq,monthlychoice,reg_name,dataset,iterations[0]),YpredTest)
        # np.savetxt(directoryoutput + 'obsout_Century_%s_%s_%s_%s-%s_iterations%s_v2.txt' % (variq,monthlychoice,reg_name,dataset_obs,dataset,iterations[0]),YpredObs)
        ...
        ##############################################################################
        ##############################################################################
        def netcdfLRP(lats,lons,var,directory,typemodel):
            print('\n>>> Using netcdfLRP function!')
            
            from netCDF4 import Dataset
            import numpy as np
            
            name = 'LRP_Maps_%s_AnnualT2M.nc' % typemodel
        ...
        netcdfLRP(lats,lons,lrptrain,directoryoutput,'train')
        netcdfLRP(lats,lons,lrptest,directoryoutput,'test')
