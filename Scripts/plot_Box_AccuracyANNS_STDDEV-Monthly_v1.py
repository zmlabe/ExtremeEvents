...
trainacc = np.genfromtxt(directorydata + 'train_totalaccuracy_STDDEVClassCentury_ANNv1-Monthly_%s_%s_%s.txt' % (dataset,reg_name,variq[0]),
              unpack=True).transpose()
trainper = np.genfromtxt(directorydata + 'train_periodaccuracy_STDDEVClassCentury_ANNv1-Monthly_%s_%s_%s.txt' % (dataset,reg_name,variq[0]),
              unpack=True).transpose()
testacc = np.genfromtxt(directorydata + 'test_totalaccuracy_STDDEVClassCentury_ANNv1-Monthly_%s_%s_%s.txt' % (dataset,reg_name,variq[0]),
              unpack=True).transpose()
testper = np.genfromtxt(directorydata + 'test_periodaccuracy_STDDEVClassCentury_ANNv1-Monthly_%s_%s_%s.txt' % (dataset,reg_name,variq[0]),
              unpack=True).transpose()
...
plt.savefig(directoryfigure + 'ClassCentury_STDDEV-Monthly_Accuracy_BOX_ANNv1_%s_%s_%s.png' % (dataset,variq[0],reg_name),
            dpi=300)
