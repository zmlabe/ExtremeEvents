...
for sis,singlesimulation in enumerate(datasetsingle):
    ...
    for seas in range(len(seasons)):
        ...
        np.savetxt(directoryoutput + 'training_%s_%s_%s_%s_iterations%s.txt' % (variq,monthlychoice,reg_name,dataset,iterations[0]),YpredTrain)
        np.savetxt(directoryoutput + 'testing_%s_%s_%s_%s_iterations%s.txt' % (variq,monthlychoice,reg_name,dataset,iterations[0]),YpredTest)
        np.savetxt(directoryoutput + 'obsout_%s_%s_%s_%s_iterations%s.txt' % (variq,monthlychoice,reg_name,dataset_obs,iterations[0]),YpredObs)
